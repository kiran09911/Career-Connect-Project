...
const CandidateDashboard = () => {
  ...
  // Enhanced Notification states (frontend-only)
  const [notifications, setNotifications] = useState([]);
  const [notificationAnchorEl, setNotificationAnchorEl] = useState(null);
  const [unreadCount, setUnreadCount] = useState(0);
  const [notificationLoading, setNotificationLoading] = useState(false);
  ...
  // Create notification when applying to a job (frontend-only)
  const createApplicationNotification = async (jobId, jobTitle, company) => {
    if (!currentUser?.id) return;
    ...
    try {
      ...
      // Update state
      setNotifications((prev) => [notification, ...prev].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)));
      setUnreadCount((prev) => prev + 1);
  ...
  useEffect(() => {
    if (currentUser?.id) {
      ...
      // Fetch applications (still needed for status changes)
      fetchApplications();
    }
  }, [currentUser?.id]);

  // Periodically refresh applications (notifications updated via status changes)
  useEffect(() => {
    if (currentUser?.id) {
      const interval = setInterval(() => {
        fetchApplications();
  ...
  const handleApply = async (jobId, recruiterId) => {
    try {
      ...
      // Create application notification (frontend-only)
      await createApplicationNotification(jobId, job?.title || "Unknown Position", job?.company || "Unknown Company");
  ...
  // Enhanced mark notification as read (frontend-only)
  ...
  // Enhanced clear all notifications (frontend-only)
  const clearAllNotifications = () => {
    if (notifications.length === 0) return;
        ...
        <Box sx={{ p: 3, borderBottom: "1px solid #e0e0e0", bgcolor: "#f8f9fa" }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
            ...
            <Box sx={{ display: "flex", gap: 1 }}>
              {unreadCount > 0 && (
                <Tooltip title="Mark all as read">
                  <IconButton size="small" onClick={markNotificationAsRead} sx={{ color: "#1976d2" }}>
